...
#ifdef _LP64
	// ASERT(((uintptr_t)new_data & 0x7) == 0);
#endif
	...
	int direction)
{
	...
	// ASERT(new_data != NULL);
	// ASERT(here != NULL);
	// ASERT(direction == AVL_BEFORE || direction == AVL_AFTER);
	...
	// ASERT(node->avl_child[child] == NULL);
...
avl_update_lt(avl_tree_t *t, void *obj)
{
	...
	// ASERT(((neighbor = AVL_NEXT(t, obj)) == NULL) ||
...
avl_update_gt(avl_tree_t *t, void *obj)
{
	...
	// ASERT(((neighbor = AVL_PREV(t, obj)) == NULL) ||
	//    (t->avl_compar(obj, neighbor) >= 0));
...
avl_swap(avl_tree_t *tree1, avl_tree_t *tree2)
{
	...
	// ASERT3P(tree1->avl_compar, ==, tree2->avl_compar);
	// ASERT3U(tree1->avl_offset, ==, tree2->avl_offset);
    ...
    size_t size, size_t offset)
{
	...
	// ASERT(compar);
	// ASERT(size > 0);
	// ASERT(size >= offset + sizeof (avl_node_t));
#ifdef _LP64
	// ASERT((offset & 0x7) == 0);
#endif
...
check_right_side:
	if (node->avl_child[1] != NULL) {
		// ASERT(AVL_XBALANCE(node) == 1);
		parent = node;
		node = node->avl_child[1];
		// ASERT(node->avl_child[0] == NULL &&
		//    node->avl_child[1] == NULL);
	} else {
		// ASERT(AVL_XBALANCE(node) <= 0);
	}

done:
	if (parent == NULL) {
		*cookie = (void *)CHILDBIT;
		// ASERT(node == tree->avl_root);
	} else {
